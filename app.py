    ...
    if script_choice == "Acurácia":
        ...
        if uploaded_file is not None:
            try:
                ...
                # Mapeamento de nomes de colunas usando palavras-chave
                def find_column(df_columns, keywords):
                    for col in df_columns:
                        if all(keyword.upper() in col for keyword in keywords):
                            return col
                    return None

                data_col = find_column(df.columns, ['CARIMBO'])
                tipo_abastecimento_col = find_column(df.columns, ['TIPO', 'ABASTECIMENTO'])
                placa_col = find_column(df.columns, ['PLACA'])
                km_col = find_column(df.columns, ['KM', 'QUILOMETRAGEM'])
                litros_col = find_column(df.columns, ['LITROS', 'VOLUME'])
                motorista_col = find_column(df.columns, ['MOTORISTA', 'RESPONSÁVEL'])
                
                df_renamed = pd.DataFrame()
                
                if data_col:
                    df_renamed['DATA ABASTECIMENTO'] = pd.to_datetime(df[data_col], errors='coerce').dt.date
                    df_renamed['HORÁRIO'] = pd.to_datetime(df[data_col], errors='coerce').dt.time
                else:
                    st.warning("Aviso: Coluna essencial 'Carimbo de data/hora' não foi encontrada. O processamento pode estar incompleto.")
                    df_renamed['DATA ABASTECIMENTO'] = np.nan
                    df_renamed['HORÁRIO'] = np.nan

                if tipo_abastecimento_col:
                    df_renamed['TIPO DE ABASTECIMENTO'] = df[tipo_abastecimento_col].astype(str).replace({'0': 'DIESEL', '1': 'ARLA', 'nan': np.nan})
                else:
                    st.warning("Aviso: Coluna essencial 'Qual o tipo de abastecimento?' não foi encontrada. O processamento pode estar incompleto.")
                    df_renamed['TIPO DE ABASTECIMENTO'] = np.nan
                
                # Mapeamento de outras colunas
                if placa_col:
                    df_renamed['PLACA'] = df[placa_col]
                else:
                    df_renamed['PLACA'] = np.nan

                if km_col:
                    df_renamed['KM'] = pd.to_numeric(df[km_col], errors='coerce')
                else:
                    df_renamed['KM'] = np.nan

                if litros_col:
                    df_renamed['LITROS'] = pd.to_numeric(df[litros_col], errors='coerce')
                else:
                    df_renamed['LITROS'] = np.nan

                if motorista_col:
                    df_renamed['MOTORISTA'] = df[motorista_col]
                else:
                    df_renamed['MOTORISTA'] = np.nan
