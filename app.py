    ...
    if script_choice == "Acurácia":
        ...
        if uploaded_file is not None:
            try:
                ...
                # Leitura do arquivo com base na extensão, como no seu script
                ...
                df.columns = [col.upper().strip().replace('HORA', 'HORÁRIO').replace('RESPONSÁVEL', 'MOTORISTA') for col in df.columns]
                ...
                # Converte tipos numéricos, ignorando erros
                ...
                if not df_diesel.empty:
                    ...
                    with pd.ExcelWriter(excel_data_diesel, engine='openpyxl') as writer:
                        for placa in placas_diesel:
                            ...
                            df_placa.sort_values(by=['DATA ABASTECIMENTO'], ascending=True, inplace=True)
...
def ti_page():
    st.title("Setor de TI")
    st.markdown("Bem-vindo(a) ao setor de TI. Abaixo estão os scripts disponíveis para análise.")

    script_choice = st.selectbox(
        "Selecione um script para executar:",
        ("Selecione...", "Importação e Download de Dados")
    )
